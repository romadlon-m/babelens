-- Flip admin_review_queue_by_article()'s ordering from oldest-first to
-- newest-first, per explicit admin request (2026-09-20) — the original
-- oldest-first choice mirrored the old per-jenis admin_review_queue()'s
-- "systematic front-to-back" rationale, but for this combined per-article
-- view the admin wants to prioritize the most recently labeled articles
-- first instead. Signature unchanged, so create or replace is safe; only the
-- final ORDER BY changes (asc -> desc).
