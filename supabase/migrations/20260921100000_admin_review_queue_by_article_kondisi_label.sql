-- Adds p_kondisi_label ('lolos_tidak' | null=semua) to the Review Label
-- per-article queue. 'lolos_tidak' = Screener Lolos (screener_passed = true)
-- AND lu_relevan = 'Tidak' AND pengeluaran_relevan = 'Tidak' — the case where
-- Pengecekan Awal saw an economic signal but both downstream stages rejected
-- it, which is the main thing an admin wants to double-check. Signature grows
-- (trailing param), so drop-then-create like every other signature change to
-- these functions (avoids an ambiguous second overload from PostgREST/RPC's
-- named-parameter dispatch).
drop function if exists admin_review_queue_by_article(int, int, text, text);
