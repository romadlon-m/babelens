-- Permanent batch membership on news.batch, so an article that starts as Batch 1
-- stays Batch 1 after labelers work on it (the old status-based predicate --
-- lu_relevan/pengeluaran_relevan both null -- moved every labeled article to
-- "Batch 2" the moment it was done, which made "finish and review Batch 1"
-- impossible to track). batch1 = genuinely new work (default for every newly
-- scraped row), batch2 = the legacy pre-tool Copilot batch.
--
-- Backfill: batch2 iff the row already has lu_relevan/pengeluaran_relevan AND
-- has no real (source = 'labeling', non-snapshot) Lapus/Pengeluaran
-- labeling_log entry. Legacy rows never enter the Lapus/Pengeluaran queue (it
-- requires those columns to be null), so a real tool entry there proves the
-- row was batch1 when labeled; 'review' entries and relabel_reset_batch
-- snapshots (hasil = '{}') are deliberately not counted.
alter table news add column if not exists batch text not null default 'batch1'
  check (batch in ('batch1', 'batch2'));

update news n set batch = 'batch2'
where (n.lu_relevan is not null or n.pengeluaran_relevan is not null)
  and not exists (
    select 1 from labeling_log l
    where l.news_id = n.id
      and l.jenis in ('lapus', 'pengeluaran')
      and l.source = 'labeling'
      and l.hasil <> '{}'::jsonb
  );

-- admin_review_queue_by_article(): same signature as 20260921100000, only the
-- batch predicate now reads the permanent column.
create or replace function admin_review_queue_by_article(
  p_page int default 1,
  p_page_size int default 10,
  p_completeness text default null,
  p_batch text default null,
  p_kondisi_label text default null
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if p_kondisi_label is not null and p_kondisi_label not in ('lolos_tidak') then
    raise exception 'kondisi label tidak valid: %', p_kondisi_label;
  end if;
  ...
  with base as (
    ...
    where p_batch is null or n.batch = p_batch
  ),
  scored as (
    select b.*,
  ...
  filtered as (
    ...
    where (s_status in ('ditandai', 'perlu_relabel', 'perlu_review')
      ...
      and (p_kondisi_label is null
        or (p_kondisi_label = 'lolos_tidak'
            and screener_passed is true
            and lu_relevan = 'Tidak'
            and pengeluaran_relevan = 'Tidak'))
  ),
  paged as (
...
revoke all on function admin_review_queue_by_article(int, int, text, text, text) from public;
grant execute on function admin_review_queue_by_article(int, int, text, text, text) to authenticated;
