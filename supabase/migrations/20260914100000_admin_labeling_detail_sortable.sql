-- Detail Baris tab (admin-users.html) is server-side paginated, so client-side
-- sort would only reorder the current 20-row page. Add p_sort_col/p_sort_dir so
-- sorting applies across the whole filtered set, matching the other 3 admin
-- tables (Sudah Pernah Login/Seluruh Pegawai and Aktivitas Labeling) which are
-- small enough to sort client-side.
--
-- labeler_nama is moved earlier into `scoped` (a plain correlated subquery, same
-- shape as the one already used for is_flagged) so it's available as an orderable
-- column before paging happens, instead of only being computed after LIMIT/OFFSET
-- in the final jsonb_build_object() like the previous version did. status_text is
-- likewise computed pre-paging (mirrors the labels used by renderDetailRows() in
-- admin-users.js) so "Status" can be sorted on the same text the admin sees.
--
-- No dynamic SQL/EXECUTE: sort column/direction are selected via a fixed set of
-- `case when v_sort_col = '...' and v_sort_dir = '...' then <col> end asc/desc`
-- clauses — every clause but the matching one evaluates to NULL for every row
-- (so it doesn't affect ordering), which avoids any identifier-injection concern
-- from interpolating p_sort_col/p_sort_dir into SQL text.
drop function if exists admin_labeling_detail(text, text, uuid, int, int);
...
revoke all on function admin_labeling_detail(text, text, uuid, int, int, text, text) from public;
grant execute on function admin_labeling_detail(text, text, uuid, int, int, text, text) to authenticated;
