-- Detail Baris' existing p_date_from/p_date_to filters news.publication_datetime
-- (when the article was published), not when it was actually labeled — no
-- existing filter answers "what did labelers submit on day D", which is what
-- a daily labeling-progress report needs to build a "download today's batch"
-- link into. Adds p_submitted_from/p_submitted_to (WIB calendar day) filtering
-- on the latest labeling_log entry per row (the same `ll` lateral join already
-- used for labeler_nama/alasan/prompt_versi) — separate params, existing
-- publication-date filter is untouched. Signature grows, so drop-then-create
-- like every prior change to this function's params (avoids an ambiguous
-- second overload from PostgREST/RPC's named-parameter dispatch).
drop function if exists admin_labeling_detail(text, text, uuid, int, int, text, text, text, text, date, date, text);
...
revoke all on function admin_labeling_detail(text, text, uuid, int, int, text, text, text, text, date, date, text, date, date) from public;
grant execute on function admin_labeling_detail(text, text, uuid, int, int, text, text, text, text, date, date, text, date, date) to authenticated;
