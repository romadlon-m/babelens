-- Detail Baris: surface each row's `summary` so admins reviewing a label's
-- "alasan" (now shown in a merged full-width row in the UI, see
-- admin-users.js renderDetailRows()) can read the article summary
-- without leaving the table. Signature is unchanged from 20260914170000
-- (only the CTEs' select lists and the final jsonb_build_object gain a
-- field), so `create or replace` is safe here — no new overload risk.
